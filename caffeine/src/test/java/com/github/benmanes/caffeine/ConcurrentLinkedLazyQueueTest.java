...
import static com.github.benmanes.caffeine.IsValidConcurrentLinkedLazyQueue.validate;
...
import com.github.benmanes.caffeine.ConcurrentLinkedLazyQueue.LinearizableNode;
import com.github.benmanes.caffeine.ConcurrentLinkedLazyQueueTest.ValidatingQueueListener;
...
public class ConcurrentLinkedLazyQueueTest {
  ...
  @Test(dataProvider = "populated")
  public void peek_whenPopulated(ConcurrentLinkedLazyQueue<Integer> queue) {
  ...
  @Test(dataProvider = "populated")
  public void element_whenPopulated(ConcurrentLinkedLazyQueue<Integer> queue) {
    Integer first = queue.head.next.value;
  ...
  @Test(dataProvider = "empty")
  public void drain_whenEmpty(ConcurrentLinkedLazyQueue<Integer> queue) {
    assertThat(queue.drain(), is(empty()));
  }

  @Test(dataProvider = "populated")
  public void drain_whenPopulated(ConcurrentLinkedLazyQueue<Integer> queue) {
    Collection<Integer> removed = queue.drain();
    assertThat(removed, hasSize(POPULATED_SIZE));
  ...
  @Test(dataProvider = "empty")
  public void noProducers_manyConsumers(Queue<Integer> queue) {
    Phaser startGate = new Phaser(NUM_CONSUMERS);
    AtomicInteger finished = new AtomicInteger();

    IntStream.range(0, NUM_CONSUMERS * PRODUCE).boxed().forEach(queue::add);
    ConcurrentTestHarness.execute(NUM_CONSUMERS, () -> {
    ...
    Awaits.await().untilAtomic(finished, is(NUM_CONSUMERS));
    assertThat(queue, is(deeplyEmpty()));
  }
  ...
  public void manyProducers_manyConsumers(Queue<Integer> queue) {
    ...
    ConcurrentTestHarness.execute(NUM_PRODUCERS, () -> {
      for (int i = 0; i < PRODUCE; i++) {
        queue.add(i);
      }
      finished.incrementAndGet();
    ...
    Awaits.await().untilAtomic(finished, is(NUM_PRODUCERS + NUM_CONSUMERS));
    assertThat(queue, is(deeplyEmpty()));
  }
  ...
  static ConcurrentLinkedLazyQueue<Integer> makePopulated(int size, boolean optimistic) {
    ConcurrentLinkedLazyQueue<Integer> queue = optimistic
        ? ConcurrentLinkedLazyQueue.optimistic()
        : ConcurrentLinkedLazyQueue.linearizable();
    populate(queue, size);
    return queue;
  }
      ...
      try {
        if (testResult.isSuccess()) {
          for (Object param : testResult.getParameters()) {
            if (param instanceof ConcurrentLinkedLazyQueue<?>) {
              assertThat((ConcurrentLinkedLazyQueue<?>) param, is(validate()));
            }
          }
        }
  ...
  static void cleanUp(ITestResult testResult) {
    ...
    for (int i = 0; i < params.length; i++) {
      ...
      if ((param instanceof ConcurrentLinkedLazyQueue<?>)) {
        ConcurrentLinkedLazyQueue<?> queue = (ConcurrentLinkedLazyQueue<?>) param;
        boolean linearizable = queue.factory.apply(null) instanceof LinearizableNode<?>;
